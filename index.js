...
var words_without_stopwords = [];
var unique_words = [];
var wordCountSentences = 0;
var wordCountSentencesLength = 0;

const prettify = function (documents) {
    ...
    var doc = documents.replace(/[.,]/g, '');
    let document_in_lowercase = doc.split(" ").map(function (x) { return x.toLowerCase(); });
    return document_in_lowercase.filter(x => !stopwords.includes(x));
};

const uniqueWords = function (words) {
    const unique_words_set = new Set(words); return unique_words = Array.from(unique_words_set);
};
...
const countWords = function (words) {
    ...
    const unique_words = uniqueWords(words); let dict = {};
...
const termFrequency = function (documents) {
...
const inverseDocumentFrequency = function (documents) {
...
const TFIDF = function (buf) {
    const documents = buf.toString('ascii');
    // calculates TF*IDF 
